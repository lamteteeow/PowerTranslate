...
- [x] Test with expired/invalid API keys
...
- [x] Replace placeholder assets with final branded PNGs at the same filenames (to avoid manifest path changes)
- [x] Add a pre-release check that blocks submission if any placeholder-marked filename is still pending replacement
- [x] Verify transparency and store-required dimensions for each replacement asset
...
- [x] ARM64 validation not in scope for this release (x64-only)
...
- [x] Create 3 screenshots showing:
  - Translation UI
  - Language selection
  - Settings page
...
- [x] Tag release as v1.0.0.0 in git
- [x] Build final MSIX package (x64)
- [x] Build final MSIX bundle (x64 + ARM64) for Store upload
